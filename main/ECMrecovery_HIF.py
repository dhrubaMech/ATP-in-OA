...
for i in range(1,len(ts)):
    ...
    if ts[i] > tstart + duration and ts[i] < tstart + duration + Rduration:
        H[i] = H[i-1] + dt*dHdt(a[i-1],H[i-1],aw,rate*bw,ac,n) # hif
    else:
...
np.savetxt('data_'+str(rate)+'_'+str(Rduration)+'.dat',data2save)
