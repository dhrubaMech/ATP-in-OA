...
gos = np.linspace(0,0.5,100)
#gos = np.logspace(-3,-0.1,20)

ATP = 0*gos

for gind, go in enumerate(gos):
    ...
    ATP[gind] = a[-1]
    print(gind, rho0, gi[-1],a[-1],E[-1],M[-1],G[-1],Lam[-1],H[-1])
    plt.plot(go,a[-1],'k.')
plt.show()    

data2save = np.zeros((len(gos),2))
data2save[:,0] = gos
data2save[:,1] = ATP
np.savetxt('ATP_rho_'+str(rho0)+'.dat',data2save)
